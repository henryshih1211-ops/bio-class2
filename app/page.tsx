...
import MobileNav, { type MobileView } from './mobile-nav';
        ...
        <section id="schedule" className="schedule panel">
          ...
          <div className="weekbar">
            <div>
              <p className="week-title">
                第 <strong>{String(week).padStart(2, '0')}</strong> 周
              </p>
              <p className="muted">
            ...
            <div className="week-controls">
              ...
              >
                <SelectTrigger aria-label="选择教学周" className="week-select">
                  <SelectValue>{'第' + week + '周'}</SelectValue>
                </SelectTrigger>
                <SelectContent>
          ...
          {active.length === 0 && (
            <Empty className="quiet">
              ...
              <p>
                {week === 2 || week === 3
                  ? '第2—3周军事训练，侯永刚老师负责。具体时间、地点待学校通知。'
                  : week < 4
                    ? '原始课表中的固定课堂教学从第4周开始，请留意学校后续通知。'
                    : '原始课表未列出本周的固定课程，请留意学校的考试及其他安排。'}
              </p>
              {week < 4 && (
                <button className="primary-button" onClick={() => setWeek(4)}>
                  查看第4周课表 <ChevronRight size={16} />
                </button>
              )}
            </Empty>
          )}
              ...
              {['周一', '周二', '周三', '周四', '周五', '周六', '周日'].map(
                (day, i) => (
                  <section key={day} className="day">
                    <header>
                      <h3>{day}</h3>
                      <span>{weekDate(week, i)}</span>
                    </header>
                    {active
                      .filter((c) => c.day === i + 1)
                      .map((c) => (
                        <article className={'course ' + c.kind} key={c.id}>
          ...
          <div className="schedule-note">
            <Info size={17} />
            <p>
              英语：周三7–8节、周五3–4节；体育：周四3–4节。分班、周次及地点待确认，未计入上方课表。节次对应的钟点时间尚未提供。
            </p>
          </div>
        </section>
        <aside className="side">
          <section className="panel note-panel">
            <p className="eyebrow">学期备忘</p>
            <h2>开学前，先看这里</h2>
            <div className="memo">
              <span>01</span>
              <div>
          ...
          <section className="panel provenance">
            ...
            <p className="muted">
              依据你提供的学校课表整理。调课与临时安排以学校最新通知为准。
            </p>
            <div className="data-label">
              27条排课记录 · 含3条待定板块与1条军训
            </div>
          </section>
        </aside>
      </div>
